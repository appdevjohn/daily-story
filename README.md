...
Every day at midnight, a new set of stories will be generated for supported languages and stored in a `stories` folder. When accessing the page `/:language/:level`, the story for the day is fetch and rendered. The stories and questions are generated with a LLM, which right now is hardcoded to a locally running LLM.
